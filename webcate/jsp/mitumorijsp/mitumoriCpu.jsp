...
<body>
	...
	<main>
		<div class="main_search">
			<form method="GET" action="MitumoriCpu">
			...
			<div class="main_sub">
				...
				<div class="scroll_box">
					<table rules="all">
						...
						int totalItems = 0;
						for (Cpu cd : cdlist) {
							if (searchKeyword == null || cd.getCpu_name().toLowerCase().contains(searchKeyword.toLowerCase())) {
								totalItems++;
							}
						}
						...
						<tr>
							<form action="Detail" method="post">
								<input type="hidden" name="goodsName"
									value="<%=cd.getCpu_name()%>">
								<td rowspan="2">
									<button type="submit"
										style="border: none; background: none; padding: 0; margin: 0;">
										<img src="./img/<%=cd.getCpu_img()%>"
											style="width: 100px; height: 100px;">
									</button>
								</td>
							</form>
							<td width="950"><%=cd.getCpu_name()%></td>
							<td><%=cd.getCpu_price()%>円</td>
						</tr>
						<tr>
							<td>説明:プロセッサー/<%=cd.getCpu_processor()%>,コア数/<%=cd.getCpu_cores()%>.スレッド数/<%=cd.getCpu_thread()%></td>
							<form action="MitumoriCpu" method="post">
								<input type="hidden" name="goodsName"
									value="<%=cd.getCpu_name()%>"> <input type="hidden"
									name="price" value="<%=cd.getCpu_price()%>">
