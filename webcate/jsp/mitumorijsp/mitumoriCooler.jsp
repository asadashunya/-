...
<body>
	...
	<main>
		<div class="main_search">
			<form method="GET" action="MitumoriCooler">
			...
			<div class="main_sub">
				...
				<div class="scroll_box">
					<table rules="all">
						...
						// 検索条件に一致するアイテムの総数を計算
						int totalItems = 0;
						for (Cooler cld : cldlist) {
							if (searchKeyword == null || cld.getCooler_name().toLowerCase().contains(searchKeyword.toLowerCase())) {
								totalItems++;
							}
						}
						...
						<tr>
							<form action="Detail" method="post">
								<input type="hidden" name="goodsName"
									value="<%=cld.getCooler_name()%>">
								<td rowspan="2">
									<button type="submit"
										style="border: none; background: none; padding: 0; margin: 0;">
										<img src="./img/<%=cld.getCooler_img()%>"
											style="width: 100px; height: 100px;">
									</button>
								</td>
							</form>
							<td width="950"><%=cld.getCooler_name()%></td>
							<td><%=cld.getCooler_price()%>円</td>
						</tr>
						<tr>
							<td>説明:ファン数/<%=cld.getCooler_fans()%>,製品会社/<%=cld.getCooler_maker()%>,電力/<%=cld.getCooler_power()%></td>
							<form action="MitumoriCooler" method="post">
								<input type="hidden" name="goodsName"
									value="<%=cld.getCooler_name()%>"> <input type="hidden"
									name="price" value="<%=cld.getCooler_price()%>">
