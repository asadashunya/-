...
<body>
	...
	<main>
		<div class="main_search">
			<form method="GET" action="MitumoriGraphic">
				...
				<div class="scroll_box">
					<table rules="all">
						...
						for (Graphic_board gbd : gbdlist) {
							if (searchKeyword == null || gbd.getGraphic_name().toLowerCase().contains(searchKeyword.toLowerCase())) {
								totalItems++;
							}
						}
						...
						for (int i = startIndex; i < endIndex; i++) {
							Graphic_board gbd = gbdlist.get(i);
							// アイテムが検索条件に一致するか確認
							if (searchKeyword == null || gbd.getGraphic_name().toLowerCase().contains(searchKeyword.toLowerCase())) {
								displayedItems++;
						%>
						...
						<tr>
							<form action="Detail" method="post">
								<input type="hidden" name="goodsName"
									value="<%=gbd.getGraphic_name()%>">
								<td rowspan="2">
									<button type="submit"
										style="border: none; background: none; padding: 0; margin: 0;">
										<img src="./img/<%=gbd.getGraphic_img()%>"
											style="width: 100px; height: 100px;">
									</button>
								</td>
							</form>
							<td width="950"><%=gbd.getGraphic_name()%></td>
							<td><%=gbd.getGraphic_price()%>円</td>
						</tr>
						<tr>
							<td>説明:メモリ/<%=gbd.getGraphic_memory()%>,製品会社/<%=gbd.getGraphic_maker()%>,電力/<%=gbd.getGraphic_power()%></td>
							<form action="MitumoriGraphic" method="post">
								<input type="hidden" name="goodsName"
									value="<%=gbd.getGraphic_name()%>"> <input type="hidden"
									name="price" value="<%=gbd.getGraphic_price()%>">
