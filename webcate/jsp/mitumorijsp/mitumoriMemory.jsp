...
<body>
	...
	<main>
		<div class="main_search">
			<form method="GET" action="MitumoriMemory">
				...
				<div class="scroll_box">
					<table rules="all">
						...
						for (Memory md : mdlist) {
							if (searchKeyword == null || md.getMemory_name().toLowerCase().contains(searchKeyword.toLowerCase())) {
								totalItems++;
							}
						}
						...
						<tr>
							<form action="Detail" method="post">
								<input type="hidden" name="goodsName"
									value="<%=md.getMemory_name()%>">
								<td rowspan="2">
									<button type="submit"
										style="border: none; background: none; padding: 0; margin: 0;">
										<img src="./img/<%=md.getMemory_img()%>"
											style="width: 100px; height: 100px;">
									</button>
								</td>
							</form>
							<td width="950"><%=md.getMemory_name()%></td>
							<td><%=md.getMemory_price()%>円</td>
						</tr>
						<tr>
							<td>説明:メモリ容量/<%=md.getMemory_size()%>GB,製品会社/<%=md.getMemory_maker()%>,枚数/<%=md.getMemory_num()%></td>
							<form action="MitumoriMemory" method="post">
								<input type="hidden" name="goodsName"
									value="<%=md.getMemory_name()%>"> <input type="hidden"
									name="price" value="<%=md.getMemory_price()%>">
