			...
			<div class="main_sub">
				...
				<div class="scroll_box">
					<table rules="all">
						...
						int totalItems = 0;
						for (Box bd : bdlist) {
							if (searchKeyword == null || bd.getBox_name().toLowerCase().contains(searchKeyword.toLowerCase())) {
								totalItems++;
							}
						}
						...
						<tr>
							<form action="Detail" method="post">
								<input type="hidden" name="goodsName"
									value="<%=bd.getBox_name()%>">
								<td rowspan="2">
									<button type="submit"
										style="border: none; background: none; padding: 0; margin: 0;">
										<img src="./img/<%=bd.getBox_img()%>"
											style="width: 100px; height: 100px;">
									</button>
								</td>
							</form>
							<td width="950"><%=bd.getBox_name()%></td>
							<td><%=bd.getBox_price()%>円</td>
						</tr>
						<tr>
							<td>説明:サイズ(mm):横/縦/幅<%=bd.getBox_size()%>,製品会社/<%=bd.getBox_maker()%>,マザーボード/<%=bd.getBox_mother()%></td>
							<form action="MitumoriBox" method="post">
								<input type="hidden" name="goodsName"
									value="<%=bd.getBox_name()%>"> <input type="hidden"
									name="price" value="<%=bd.getBox_price()%>">
