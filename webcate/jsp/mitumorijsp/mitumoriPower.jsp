...
<body>
	...
	<main>
		<div class="main_search">
			<form method="GET" action="MitumoriPower">
			...
			<div class="main_sub">
				...
				<div class="scroll_box">
					<table rules="all">
						...
						int totalItems = 0;
						for (Power pd : pdlist) {
							if (searchKeyword == null || pd.getPower_name().toLowerCase().contains(searchKeyword.toLowerCase())) {
								totalItems++;
							}
						}
						...
						<tr>
							<form action="Detail" method="post">
								<input type="hidden" name="goodsName"
									value="<%=pd.getPower_name()%>">
								<td rowspan="2">
									<button type="submit"
										style="border: none; background: none; padding: 0; margin: 0;">
										<img src="./img/<%=pd.getPower_img()%>"
											style="width: 100px; height: 100px;">
									</button>
								</td>
							</form>
							<td width="950"><%=pd.getPower_name()%></td>
							<td><%=pd.getPower_price()%>円</td>
						</tr>
						<tr>
							<td>説明:80plus/<%=pd.getPower_80plus()%>,製品会社/<%=pd.getPower_maker()%>,電源容量/<%=pd.getPower_capacity()%>Ｗ</td>
							<form action="MitumoriPower" method="post">
								<input type="hidden" name="goodsName"
									value="<%=pd.getPower_name()%>"> <input type="hidden"
									name="price" value="<%=pd.getPower_price()%>">
