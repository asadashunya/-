...
<body>
	...
	<main>
		<div class="main_search">
			<form method="GET" action="MitumoriSsd">
			...
			<div class="main_sub">
				...
				<div class="scroll_box">
					<table rules="all">
						...
						int totalItems = 0;
						for (Ssd sd : sdlist) {
							if (searchKeyword == null || sd.getSsd_name().toLowerCase().contains(searchKeyword.toLowerCase())) {
								totalItems++;
							}
						}
						...
						<tr>
							<form action="Detail" method="post">
								<input type="hidden" name="goodsName"
									value="<%=sd.getSsd_name()%>">
								<td rowspan="2">
									<button type="submit"
										style="border: none; background: none; padding: 0; margin: 0;">
										<img src="./img/<%=sd.getSsd_img()%>"
											style="width: 100px; height: 100px;">
									</button>
								</td>
							</form>
							<td width="950"><%=sd.getSsd_name()%></td>
							<td><%=sd.getSsd_price()%>円</td>
						</tr>
						<tr>
							<td>説明:容量/<%=sd.getSsd_capacity()%>,製品会社/<%=sd.getSsd_maker()%>,インターフェース/<%=sd.getSsd_interface()%></td>
							<form action="MitumoriSsd" method="post">
								<input type="hidden" name="goodsName"
									value="<%=sd.getSsd_name()%>"> <input type="hidden"
									name="price" value="<%=sd.getSsd_price()%>">
